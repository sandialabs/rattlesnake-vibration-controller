...
class NonlinearAcquisition(HardwareAcquisition):
    ...
    actually simulated by integrating a state space function x_dot = f(x,t).
    It is run by the Acquisition process, and must define how to get data from
    the test hardware into the controller.
    """
    def __init__(self,python_file : str,python_name : str,queue : mp.queues.Queue):
        '''Loads in a python file and sets initial parameters to null values
        
        Parameters
        ----------
        python_file : str : 
            A python file containing a class or function that will be used to
            provide the equations of motion.
        python_name : str :
            The name of the Python function or class that will be used to
            integrate the equations of motion.
        queue : mp.queues.Queue
            A queue that passes input data from the NonlinearOutput class to
        ...
        '''
        path,file = os.path.split(python_file)
        file,ext = os.path.splitext(file)
        spec = importlib.util.spec_from_file_location(file, python_file)
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        self.ivp_class = getattr(module,python_name)
        self.times = None
...
class NonlinearOutput(HardwareOutput):
