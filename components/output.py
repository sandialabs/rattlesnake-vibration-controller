...
from .utilities import QueueContainer,GlobalCommands,flush_queue,VerboseMessageQueue
...
class OutputProcess(AbstractMessageProcess):
    ...
    def output_signal(self,data):
        ...
        for environment in self.environment_list:
            ...
            if not self.environment_active_flags[environment] or self.environment_shutting_down_flags[environment]:
                continue
        ...
        if ready_to_write and (self.startup or self.hardware.ready_for_new_output()):
...
#            np.savez('test_data/output_data_check.npz',output_data = write_data)
            ...
            if self.startup:
                ...
                self.queue_container.input_output_sync_queue.put((True))
                self.startup = False
...
def output_process(queue_container : QueueContainer,environments : list):
    ...
    output_instance = OutputProcess('Output',queue_container,environments)
