...
class SysIdSignalGenerationMessages(Enum):
...
class SysIdSignalGenerationProcess(AbstractMessageProcess):
    ...
    def __init__(self,process_name : str, instruction_queue : VerboseMessageQueue,
        ...
        self.map_command(SysIdSignalGenerationMessages.INITIALIZE_DATA_ACQUISITION,self.initialize_data_acquisition)
        self.map_command(SysIdSignalGenerationMessages.INITIALIZE_TEST_PARAMETERS,self.initialize_test_parameters)
        self.map_command(SysIdSignalGenerationMessages.RUN_TRANSFER_FUNCTION,self.run_transfer_function)
        self.map_command(SysIdSignalGenerationMessages.MUTE,self.mute)
        self.map_command(SysIdSignalGenerationMessages.ADJUST_TEST_LEVEL,self.adjust_test_level)
        self.map_command(SysIdSignalGenerationMessages.START_SHUTDOWN,self.start_shutdown)
        self.map_command(SysIdSignalGenerationMessages.QUIT_EVENTUALLY,self.quit_eventually)
    ...
    def run_transfer_function(self,data):
        ...
        if self.startup:
            ...
            self.controller_communication_queue.put(self.process_name,(GlobalCommands.RUN_HARDWARE,None))
            self.controller_communication_queue.put(self.process_name,(GlobalCommands.START_ENVIRONMENT,self.environment_name))
            self.startup=False
        if self.data_out_queue.empty(): # May need to change this to some kind of buffer if it takes longer to COLA the signal than it does to output the signal
            ...
            if last_run:
                self.shutdown()
                return
        self.instruction_queue.put(self.process_name,(SysIdSignalGenerationMessages.RUN_TRANSFER_FUNCTION,None))
    ...
    def start_shutdown(self,data):
        ...
        self.instruction_queue.put(self.process_name,(SysIdSignalGenerationMessages.ADJUST_TEST_LEVEL,0.0))
        # Put the run command back onto the stack.
        if SysIdSignalGenerationMessages.RUN_TRANSFER_FUNCTION in commands:
            self.instruction_queue.put(self.process_name,(SysIdSignalGenerationMessages.RUN_TRANSFER_FUNCTION,None))
