...
def srs(
    ...
    damping=0.05,
    spectrum_type=9,
    b_filter_weights=None,
    a_filter_weights=None,
):
    """
    ...
    if b_filter_weights.shape != (frequencies.size, 3):
        raise ValueError(
            "`b_filter_weights` and `a_filter_weights` must have shape (frequencies.size,3)"
        )
    ...
    for i_freq, (frequency, b, a) in enumerate(
        ...
        residual_responses = np.concatenate(
            [
                response[..., num_samples, np.newaxis],
...
def sdof_ramp_invariant_filter_weights(frequencies, sample_rate, damping, spectrum_type):
    ...
    if spectrum_type > 0:
        ...
        # z*w + (w*w)*( (1/6) - 2*z*z/3 );
        b[small_freq_indices, 0] = damping * small_freqs + small_freqs**2 * (
            1 / 6 - 2 * damping**2 / 3
        )
        # 2*w*w*(1-z*z)/3
        b[small_freq_indices, 1] = 2 * small_freqs**2 * (1 - damping**2) / 3
        # -z*w + w*w*( (1/6) - 4*z*z/3 );
        b[small_freq_indices, 2] = -damping * small_freqs + small_freqs**2 * (
            1 / 6 - 4 * damping**2 / 3
        )
    else:
    ...
    if spectrum_type > 0:
        ...
        b[~small_freq_indices, 2] = (
            -(e**2 * (large_freqs + 2 * damping) - 2 * damping * c + fact) / large_freqs
        )
...
def sdof_free_decay_peak_response(responses, times_at_responses, frequency, damping):
    ...
    tmax = np.array(
        (1 / wd)
        * np.arctan(
            (wd * z[..., 0] - damping * wn * z[..., 1])
            / (wd * z[..., 1] - damping * wn * z[..., 0])
        )
    )
    ...
    response_peaks.append(
        np.exp(-damping * wn * tmax)
        * (z[..., 0] * np.sin(wd * tmax) + z[..., 1] * np.cos(wd * tmax))
    )
    tmin = tmax + np.pi / wd
    response_peaks.append(
        np.exp(-damping * wn * tmin)
        * (z[..., 0] * np.sin(wd * tmin) + z[..., 1] * np.cos(wd * tmin))
    )
...
def sum_decayed_sines(
    sample_rate,
    block_size,
    sine_frequencies=None,
    sine_tone_range=None,
    sine_tone_per_octave=None,
    sine_amplitudes=None,
    sine_decays=None,
    sine_delays=None,
    required_srs=None,
    srs_breakpoints=None,
    srs_damping=0.05,
    srs_type=9,
    ...
    tau=None,
    num_time_constants=None,
    decay_resolution=None,
    scale_factor=1.02,
    acceleration_factor=1.0,
    srs_frequencies=None,
        ...
        for freq in sine_frequencies:
            if isinstance(tau, dict):
                ...
                if this_decay is None:
                    raise ValueError(
                        f"No frequency range matching frequency {freq} "
                        "was found in the specified decay parameters."
                    )
    ...
    (
        ...
        convergence,
        error_tolerance,
        ignore_compensation_pulse,
        verbose,
    )

    all_frequencies = np.concatenate((used_frequencies, [used_comp_frequency]))
    all_amplitudes = np.concatenate((used_amplitudes, [used_comp_amplitude]))
    all_decays = np.concatenate((used_decays, [used_comp_decay]))
    all_delays = np.concatenate((used_delays, [used_comp_delay]))
...
def _sum_decayed_sines(
    ...
    for i in range(number_of_iterations):
        ...
        (
            this_signal,
            this_compensation_frequency,
            this_compensation_amplitude,
            this_compensation_decay,
            this_compensation_delay,
        ) = sum_decayed_sines_reconstruction_with_compensation(
            sine_frequencies,
            this_sine_amplitudes,
            sine_decays,
            sine_delays,
            compensation_frequency,
            compensation_decay,
    ...
    if verbose:
        ...
        print(
            f'{"Sine":>8s}, {"Frequency":>10s}, {"Amplitude":>10s}, '
            f'{"SRS Val":>10s},  {"SRS Req":>10s}, {"Error":>10s}'
        )
        for i, (freq, amp, srs_val, srs_req, srs_err) in enumerate(
            zip(sine_frequencies, sine_amplitudes, this_srs, required_srs, srs_error)
        ):
            print(
                f"{str(i):>8s}, {freq:>10.4f}, {amp:>10.4f}, "
                f"{srs_val:>10.4f}, {srs_req:>10.4f}, {srs_err:>10.4f}"
            )
        print("Compensating Pulse:")
        print(f'{"Frequency":>10s}, {"Amplitude":>10s}, {"Decay":>10s}, {"Delay":>10s}')
        print(
            f"{this_compensation_frequency:>10.4f}, {this_compensation_amplitude:>10.4f}, "
            f"{this_compensation_decay:>10.4f}, {this_compensation_delay:>10.4f}"
        )
    return (
        this_signal,
        sine_frequencies,
...
def _sum_decayed_sines_single_iteration(
    sine_frequencies,
    sine_amplitudes,
    sine_decays,
    sine_delays,
    ...
    for i, (frequency, amplitude, decay, delay, desired_srs) in enumerate(
        zip(sine_frequencies, sine_amplitudes, sine_decays, sine_delays, required_srs)
    ):
        # if i == 10:
        #     break
        srs_error = float("inf")
        ...
        other_pulse = sum_decayed_sines_reconstruction(
            other_sine_frequencies,
            other_sine_amplitudes,
            other_sine_decays,
            other_sine_delays,
            sample_rate,
            block_size,
        ...
        while abs(srs_error) > error_tolerance:
            ...
            if not ignore_compensation_pulse:
                compensation_amplitude, compensation_delay = (
            ...
            if abs(num_shift) >= block_size:
                raise ValueError(
                    "The number of samples in the compensation delay ({:}) is larger than the block_size ({:}).".format(
                        num_shift, block_size
                    )
            ...
            if verbose:
                print(
                    "Iteration at frequency {:}, {:0.4f}\n  Iteration Count: {:}, SRS Error: {:0.4f}".format(
                        i, frequency, iteration_count, srs_error
                    )
                )
            ...
            if abs(srs_error) > error_tolerance:
                ...
                correction_slope = (abs(amplitude) - abs(new_amplitude)) / (
                    srs_prediction - srs_perturbed
                )
                if correction_slope > 0:
                    # this is what we want, it means we are changing in the right
                    # direction
                    new_amplitude = convergence * correction_slope * (
                        desired_srs - srs_prediction
                    ) + abs(amplitude)
                    ...
                    if abs(num_shift) >= block_size:
                        raise ValueError(
                            "The number of samples in the compensation delay"
                            + " ({:}) is larger than the block_size ({:}).".format(
                                num_shift, block_size
                            )
                            + "  The entire pulse will consist of part of the compensation pulse.  "
                            + "Please increase the block_size or compensation frequency."
                    ...
                    if verbose:
                        print(
                            "  Old Amplitude: {:0.4f}, New Amplitude: {:0.4f}, Error: {:0.4f}".format(
                                old_amplitude, amplitude, srs_error
                            )
                        )
                    # Don't allow amplitude to change more than a factor of 10 in an iteration
                    if np.abs(10 * old_amplitude) < np.abs(amplitude) or np.abs(
                        0.1 * old_amplitude
                    ) > np.abs(amplitude):
                        iteration_count = number_of_iterations + 1
                else:
                ...
                if iteration_count > number_of_iterations:
                    print(
                        "  Warning: SRS did not converge for frequency {:}: {:0.4f}".format(
                            i, frequency
                        )
                    )
                    break
...
def sum_decayed_sines_compensating_pulse_parameters(
    sine_frequencies,
    sine_amplitudes,
    sine_decays,
    sine_delays,
    ...
    var0 = (
        np.sum(sine_amplitudes * sine_delays / (omegas * (sine_decays**2 + 1)))
    ) / compensation_amplitude
    ...
    var1 = (
        np.sum(
            (2 * sine_decays * sine_amplitudes)[np.newaxis, :]
...
def sum_decayed_sines_displacement_velocity(
    ...
    for k in range(len(sine_frequencies)):
        ...
        tmp2[indices] = z[k] * np.sin(w[k] * (t[indices] - tau[k])) + np.cos(
            w[k] * (t[indices] - tau[k])
        )
...
def optimization_error_function(
    ...
    (
        input_signal,
        new_compensation_frequency,
        new_compensation_amplitude,
        new_compensation_decay,
    ...
    control_responses = np.array(
        [oaconvolve(control_irf, input_signal) for control_irf in control_irfs]
    )[..., :block_size]
    # Compute SRSs at all frequencies
    predicted_control_srs, frequencies = srs(
        control_responses,
        1 / sample_rate,
        sine_frequencies[frequency_index],
        srs_damping,
        srs_type,
    ...
    if limit_srs is not None:
        limit_responses = np.array(
            [oaconvolve(limit_irf, input_signal) for limit_irf in limit_irfs]
        )[..., :block_size]
        predicted_limit_srs, frequencies = srs(
            limit_responses,
            1 / sample_rate,
            sine_frequencies[frequency_index],
            srs_damping,
...
def optimization_callback(intermediate_result, rms_error_threshold=0.02, verbose=True):
    if verbose:
        print(
            "Amplitude Scale: {:}, error is {:}".format(
                intermediate_result.x.squeeze(), intermediate_result.fun
            )
        )
...
def sum_decayed_sines_minimize(
    sample_rate,
    block_size,
    sine_frequencies=None,
    sine_tone_range=None,
    ...
    limit_breakpoints=None,
    limit_transfer_functions=None,
    control_transfer_functions=None,
    control_weights=None,
    # Parameters for the optimizer
    minimize_iterations=1,
    rms_error_threshold=None,
    ...
    if control_srs is None:
        ...
        control_srs = np.array(
            [
                loginterp(sine_frequencies, frequencies, breakpoint_curve)
                for breakpoint_curve in breakpoint_curves
            ]
        )
    else:
    ...
    if sine_amplitudes is None:
        ...
        srs_amplitudes = np.array(
            [
                nnls(ai[:, np.newaxis], bi)[0][0]
                for ai, bi in zip(tf_at_frequencies.T, control_srs.T)
            ]
        )
        srs_amplitudes[np.arange(srs_amplitudes.size) % 2 == 0] *= -1
        quality = 1 / (2 * srs_damping)
    ...
    if tau is not None:
        ...
        for freq in sine_frequencies:
            if isinstance(tau, dict):
                ...
                if this_decay is None:
                    raise ValueError(
                        "No frequency range matching frequency {:} was found in the specified decay parameters.".format(
                            freq
                        )
                    )
                sine_decays.append(this_decay)
    ...
    for j in range(optimization_passes):
        for i in range(len(sine_frequencies)):
            if verbose:
                print(
                    "Pass {:}, Analyzing Frequency {:}: {:0.2f}".format(
                        j + 1, i, sine_frequencies[i]
                    )
                )
    ...
    all_frequencies = np.concatenate((sine_frequencies, [compensation_frequency]))
    all_amplitudes = np.concatenate((sine_amplitudes, [compensation_amplitude]))
    all_decays = np.concatenate((sine_decays, [compensation_decay]))
    all_delays = np.concatenate((sine_delays, [compensation_delay]))
    ...
    if plot_results:
        ...
        this_srs, this_frequencies = srs(
            input_signal, 1 / sample_rate, sine_frequencies, srs_damping, srs_type
        )
        ...
        for i, (predicted, control, response) in enumerate(
            zip(predicted_control_srs, control_srs, control_responses)
        ):
        ...
        if limit_srs is not None:
            for i, (predicted, limit, response) in enumerate(
                zip(predicted_limit_srs, limit_srs, limit_responses)
            ):
                fig, ax = plt.subplots(1, 2, figsize=(8, 3))
