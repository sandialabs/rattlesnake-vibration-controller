...
class PlotTimeWindow(QtWidgets.QDialog):
    ...
    def __init__(self, parent, index, specification, sample_rate, index_name, unit=None):
        """
...
def channel_unit_label(channel_list):
    ...
    """
    units = {c.unit for c in channel_list if c is not None and c.unit not in (None, "", "-")}
...
def axis_label(type, base, unit=None):
    ...
    match type:
        ...
        case "ratio":
            ...
            return (
                base if not (numerator and denominator) else f"{base} ({numerator}/{denominator})"
            )
        case _:
            return base
    ...
    def add_ip_address(
        ...
        if isinstance(ip_address, IPAddress):
            ...
            if len(self.ip_addresses) == previous_count:
                ...
                merged_row = next(
                    row for row, address in enumerate(self.ip_addresses) if address == ip_address
                )
...
class ModalMDISubWindow(QtWidgets.QWidget):
    ...
    def selected_response_unit(self):
        ...
        if current_signal in (
            ...
            try:
                ...
                channel = self._channel_at(self.parent.response_channel_indices[response_index])
            except (IndexError, TypeError):
                channel = None
    ...
    def update_axis_labels(self):
        ...
        if current_signal in (0, 1):  # Time, Windowed Time
            ...
            self.primary_plotitem.setLabel("bottom", "Frequency (Hz)")
            self.primary_plotitem.setLabel("left", axis_label("psd", "Autospectrum", response_unit))
