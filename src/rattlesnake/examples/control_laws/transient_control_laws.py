...
def pseudoinverse_control(
    ...
    added_zeros = np.zeros(
        (specification_signals.shape[-1] - impulse_response.shape[0],)
    ...
    drive_signals_fft_zero_padded = np.concatenate(
        (
            drive_signals_fft[:-1],
            np.zeros(
                (drive_signals_fft[:-1].shape[0] * (output_oversample_factor - 1) + 1,)
                + drive_signals_fft.shape[1:]
            ),
        ),
        axis=0,
    )
    ...
    drive_signals_oversampled = (
        np.fft.irfft(drive_signals_fft_zero_padded.T, axis=-1)
        * output_oversample_factor
    )
...
def pseudoinverse_control_generator():
    ...
    while True:
        ...
        if inverted_frf is None:
            ...
            if zero_impulse_after is not None:
                # Remove noncausal portion
                impulse_response_abscissa = (
                    np.arange(impulse_response.shape[0]) / sample_rate
                )
                zero_indices = impulse_response_abscissa > zero_impulse_after
                impulse_response[zero_indices] = 0
            ...
            full_impulse_response = np.concatenate(
                (impulse_response, added_zeros), axis=0
            )
        ...
        drive_signals_fft_zero_padded = np.concatenate(
            (
                ...
                np.zeros(
                    (
                        drive_signals_fft[:-1].shape[0] * (output_oversample_factor - 1)
                        + 1,
                    )
                    + drive_signals_fft.shape[1:]
                ),
...
class pseudoinverse_control_class:
    ...
    def system_id_update(
        ...
        if self.zero_impulse_after is not None:
            # Remove noncausal portion
            impulse_response_abscissa = (
                np.arange(impulse_response.shape[0]) / self.sample_rate
            )
        ...
        # Zero pad the impulse response to create a signal that is long enough
        added_zeros = np.zeros(
            (self.specification_signals.shape[-1] - impulse_response.shape[0],)
            + impulse_response.shape[1:]
        )
        full_impulse_response = np.concatenate((impulse_response, added_zeros), axis=0)
        ...
        # Perform convolution in frequency domain
        self.inverted_frf = np.linalg.pinv(
            interpolated_transfer_function, rcond=self.rcond
        )
        ...
        drive_signals_fft_zero_padded = np.concatenate(
            (
                ...
                np.zeros(
                    (
                        drive_signals_fft[:-1].shape[0]
                        * (self.output_oversample_factor - 1)
                        + 1,
                    )
                    + drive_signals_fft.shape[1:]
                ),
        ...
        self.drive_signals_oversampled = (
            np.fft.irfft(drive_signals_fft_zero_padded.T, axis=-1)
            * self.output_oversample_factor
        )
