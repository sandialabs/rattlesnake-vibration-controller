...
def completed_system_id(transient_environment, transient_metadata):
    ...
    data_package = mock_sysid_data_package(transient_metadata.sysid_metadata)
...
def test_transient_metadata_netcdf_round_trip(
    transient_metadata, with_transformations
):
    if with_transformations:
        transient_metadata.response_transformation_matrix = np.arange(
            2.0
        ).reshape(2, 1)
        transient_metadata.reference_transformation_matrix = np.arange(
            3.0
        ).reshape(3, 1)
...
def test_transient_environment_initialize_sysid(
    ...
    assert (
        transient_environment.environment_metadata.sysid_metadata
        == new_sysid_metadata
    )
    (analysis_message,) = get_queue_messages(
        transient_environment.queue_container.data_analysis_command_queue,
...
def test_transient_environment_system_id_complete_runs_prediction(
    ...
    for _ in range(5):
        ...
        gui_commands.append(
            gui_message[1][0]
            if gui_message[0] == ENVIRONMENT_NAME
            else gui_message[0]
        )
...
@mock.patch(
    "rattlesnake.environment.transient_sys_id_environment.threading.Thread"
)
@mock.patch(
    "rattlesnake.environment.transient_sys_id_environment.TransientEnvironment"
)
