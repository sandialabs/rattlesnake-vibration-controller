...
def environment_manager(request):
    ...
    environment_manager = EnvironmentManager(
        queue_container, event_container, use_thread
    )
...
def test_environment_manager_available_queues(
    queue_names, available_queues, environment_manager
):
...
def test_environment_manager_initialize_hardware(environment_manager):
    ...
    mock_command_queue.put.assert_called_with(
        "Environment Manager", (GlobalCommands.INITIALIZE_HARDWARE, hardware_metadata)
    )
            ...
            (
                {
                    ...
                    "Environment 1": MockEnvironmentMetadata(),
                },
                {
                    "Environment 0": MockEnvironmentType.ENVIRONMENT,
                    "Environment 1": MockEnvironmentType.ENVIRONMENT,
                },
...
def test_environment_manager_initialize_environment(
    existing_metadata, valid_metadata, expected, environment_manager
):
    ...
    if expected is RattlesnakeError:
        with pytest.raises(RattlesnakeError):
            environment_manager.initialize_environments(
                metadata_list, hardware_metadata
            )
    else:
...
def test_environment_manager_validate_environment_metadata(
    environment_name_list, instance_list, expected, environment_manager
):
    hardware_metadata = MockHardwareMetadata()
    environment_manager.queue_names = ["Environment 0", "Environment 1"]
    metadata_list = []

    for environment_name, instance in zip(
...
@pytest.mark.parametrize(
    "environment_name, environment_type, instance, expected",
    [
        (
            "Mock Environment",
            MockEnvironmentType.ENVIRONMENT,
            EnvironmentInstructions,
            True,
        ),
        (
        ...
        ("Mock Environment", None, EnvironmentInstructions, RattlesnakeError),
        (
            "Mock Environment",
            MockEnvironmentType.ENVIRONMENT,
            None,
...
def test_environment_manager_validate_instructions(
    ...
    if expected is True:
        valid_instruction = environment_manager.validate_environment_instructions(
            environment_instructions
        )
        assert valid_instruction
    elif expected is RattlesnakeError:
        with pytest.raises(RattlesnakeError):
            environment_manager.validate_environment_instructions(
                environment_instructions
            )
...
def test_environment_mananger_validate_profile_events(
    environment_name, instance, expected, environment_manager
):
    environment_manager.queue_names = ["Environment 0"]
    environment_manager.environment_names["Environment 0"] = "Environment Name"
...
#         )
#         assert (
#             environment_manager.environment_types["Environment 0"] == environment_type
#         )
#         assert (
#             environment_manager.environment_processes["Environment 0"] == mock_process
#         )
#         assert not environment_manager.environment_close_events[
#             "Environment 0"
#         ].is_set()
...
def test_environment_mananger_add_environment_error(environment_manager):
    hardware_metadata = MockHardwareMetadata()
    metadata = MockEnvironmentMetadata()
...
def test_environment_manager_remove_environment(
    mock_time, is_alive, environment_manager
):
    environment_manager.queue_names = ["Environment 0"]
    environment_manager.environment_names = {"Environment 0": "Environment Name"}
    environment_manager.environment_types = {
    ...
    mock_command_queue.put.assert_called_with(
        "Environment Manager", (GlobalCommands.QUIT, None)
    )
...
@pytest.mark.parametrize("is_alive", [True, False])
@mock.patch("rattlesnake.environment_manager.datetime")
def test_environment_mananger_close_environment(
    mock_time, is_alive, environment_manager
):
    environment_manager.queue_names = ["Environment 0"]
    environment_manager.environment_names = {"Environment 0": "Environment Name"}
