...
from rattlesnake.testing.mock_sysid import (
    drain_queue_commands,
    get_queue_messages,
    mock_sysid_data_package,
    mock_sysid_metadata,
...
def make_random_metadata():
    ...
    return RandomVibrationMetadata(
        ...
        sysid_metadata=mock_sysid_metadata(),
    )
...
def test_random_metadata_load_specification_no_limits(tmp_path, random_metadata):
    spec_path = str(tmp_path / "spec.npz")
    write_cpsd_spec_npz(
        spec_path, 5.0 * np.arange(101), 1, warning=False, abort=False
    )
...
def test_random_metadata_load_specification_off_grid_lines(
...
def test_random_metadata_netcdf_round_trip(random_metadata, with_transformations):
    if with_transformations:
        random_metadata.response_transformation_matrix = np.arange(2.0).reshape(
            2, 1
        )
        random_metadata.reference_transformation_matrix = np.arange(3.0).reshape(
            3, 1
        )
...
def test_random_environment_initialize_environment(
...
def test_random_environment_initialize_sysid(random_environment, random_metadata):
    ...
    new_sysid_metadata = mock_sysid_metadata(frame_size=100)

    random_environment.initialize_sysid(new_sysid_metadata)

    assert (
        random_environment.environment_metadata.sysid_metadata
        == new_sysid_metadata
    )
    (analysis_message,) = get_queue_messages(
        random_environment.queue_container.data_analysis_command_queue,
...
def test_random_environment_system_id_complete(random_environment, random_metadata):
    initialized_environment(random_environment, random_metadata)
    data_package = mock_sysid_data_package(random_metadata.sysid_metadata)
...
def test_random_environment_change_specification(
    ...
    while True:
        gui_messages.append(
            random_environment.gui_update_queue.get(timeout=0.5)
        )
...
def test_random_environment_run_full_lifecycle(random_environment, random_metadata):
    ...
    data_package = mock_sysid_data_package(sysid_metadata)
...
@pytest.mark.parametrize("use_thread", [True, False])
@mock.patch(
    "rattlesnake.environment.random_vibration_sys_id_environment.mp.Process"
)
@mock.patch(
